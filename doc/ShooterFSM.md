...
stateDiagram-v2
    ...
    Shooter_Prep --> Feeder : flywheelAtSpeed && rightTriggerPressed

    Passer_Prep --> Shooter_Prep : leftBumperPressed
    Passer_Prep --> Manual_Prep : leftTriggerPressed
    Passer_Prep --> Idle : touchpadPressed
    Passer_Prep --> Feeder : flywheelAtSpeed && rightTriggerPressed

    Manual_Prep --> Idle : touchpadPressed
    Manual_Prep --> Feeder : flywheelAtSpeed && rightTriggerPressed

    Feeder --> Manual_Prep : (!flywheelAtSpeed || !rightTriggerPressed) && previousState = Manual_Prep
    Feeder --> Passer_Prep : (!flywheelAtSpeed || !rightTriggerPressed) && previousState = Passer_Prep
    Feeder --> Shooter_Prep : (!flywheelAtSpeed || !rightTriggerPressed) && previousState = Shooter_Prep
    Feeder --> Idle : touchpadPressed
    Feeder --> Feeder :  flywheelAtSpeed && rightTriggerPressed
    ...
    note: 4. Every prep state handles flywheel speed. Intake state will only control the feeder speed to allow a ball to be shot
    note: 5. All states will remain in their current state with no input, with the exception being the intake state
    note: 6. Every prep state will also maintain motor speed to match new poses and desires
