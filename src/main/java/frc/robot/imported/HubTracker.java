...
public class HubTracker {
     ...
     * Will return {@link Optional#empty()} if disabled or in between auto and
     * teleop.
     */
    ...
    /**
     * Returns an {@link Optional} containing the current {@link Time} remaining in
     * the current shift.
     * Will return {@link Optional#empty()} if disabled or in between auto and
     * teleop.
     */
    ...
    /**
     * Returns whether the hub is active during the specified {@link Shift} for the
     * specified {@link Alliance}.
    ...
    /**
     * Returns whether the hub is active during the current {@link Shift} for the
     * specified {@link Alliance}.
     * Will return {@code false} if disabled or in between auto and teleop.
     */
    ...
    /**
     * Returns whether the hub is active for the next {@link Shift} for the
     * specified {@link Alliance}.
     * Will return {@code false} if disabled or in between auto and teleop.
     */
    ...
    /**
     * Returns the {@link Alliance} that won auto as specified by the FMS/Driver
     * Station's game specific message data.
     * Will return {@link Optional#empty()} if no game message or alliance is
     * available.
     */
    ...
    /**
     * Returns time remaining until the next Shift starts.
     * Will return Optional.empty() if disabled or no next shift exists.
     */
    public static Optional<Time> timeUntilNextShift() {
        double matchTime = getMatchTime();
        if (matchTime < 0)
            return Optional.empty();

        return getNextShift()
                .map(shift -> Seconds.of(shift.startTime - matchTime));
    }
