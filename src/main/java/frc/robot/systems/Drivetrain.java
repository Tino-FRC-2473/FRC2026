...
public class Drivetrain extends FSMSystem<Drivetrain.DrivetrainState> {
	...
	/**
	 * Constructs the Drivetrain subsystem and initializes the FSM state to ENTRY.
	 */
	public Drivetrain() {
	...
	public void update(Input input) {
		...
		Logger.recordOutput("Drivetrain/Target Angle Pose",
				getPose().rotateAround(getPose().getTranslation(),
	...
	private void handleFaceTarget(Input input, Pose2d targetPose) {
		...
		if (Math.abs(MathUtil.angleModulus(getRotation()
	...
	protected DrivetrainState nextState(Input input) {
		...
		switch (currentState) {
			...
			case FACE_HUB:
				return input.getButtonValue(ButtonInput.FACE_HUB) ? DrivetrainState.FACE_HUB
						: DrivetrainState.TELEOP;

			case FACE_PASS:
				return input.getButtonValue(ButtonInput.FACE_PASS)
						&& Math.abs(MathUtil.angleModulus(getRotation()
							- targetAngle) + Math.PI) >= Math.toRadians(N20.instance.getNum())
								? DrivetrainState.FACE_PASS
								: DrivetrainState.TELEOP;
	...
	private void setupAndStartPathfinding() {
		...
		int tagId = (alliance == DriverStation.Alliance.Red)
			? DrivetrainConstants.RED_TAG_ALLIGNMENT_TAG
				: DrivetrainConstants.BLUE_TAG_ALLIGNMENT_TAG;
	...
	/**
	 * Get the current pose of the drivetrain.
	...
	/**
	 * Get the current rotation of the drivetrain in radians, normalized to [0, 2pi).
	...
	/**
	 * Get the current rotation of the drivetrain in radians, normalized to [0, 2pi).
	 * @return the current rotation + 90 degrees
	 */
	@AutoLogOutput(key = "Drivetrain/Rotation")
	public double getRotation90() {
		return MathUtil.angleModulus(drivetrain.getPigeon2().
			getRotation2d().getRadians() + Math.PI + Math.PI / 2);
	}

	/**
	 * Get the current 3D rotation of the drivetrain.
	 * @return the current 3D rotation
	 */
	@AutoLogOutput(key = "Drivetrain/Rotation3d")
	public Rotation3d getRotation3d() {
		return drivetrain.getPigeon2().getRotation3d();
	}

	private boolean hasDriverInput(Input input) {
		return Math.abs(input.getAxisValue(AxialInput.DRIVETRAIN_DRIVE_X))
			> DrivetrainConstants.TRANSLATION_DEADBAND
				|| Math.abs(input.getAxisValue(AxialInput.DRIVETRAIN_DRIVE_Y))
					> DrivetrainConstants.TRANSLATION_DEADBAND
				|| Math.abs(input.getAxisValue(AxialInput.DRIVETRAIN_ROTATE))
					> DrivetrainConstants.ROTATION_DEADBAND;
	}

	/**
	 * Stop the drivetrain by setting the control to idle.
	 */
	public void stop() {
	...
	public void addVisionMeasurement(
		...
		// drivetrain.addVisionMeasurement(visionPoseMeters,
		// timestampSeconds,visionStdDevs);
	}
}
